...
PROJECT=${GCP_PROJECT:?set GCP_PROJECT (e.g. your-library-esources)}
...
: "${PUBLIC_BASE_URL:?set in .env — e.g. https://databases.yourlibrary.org}"
...
gcloud run deploy "${SERVICE}" \
  ...
  --concurrency=80 \
  --timeout=60s \
  --set-env-vars="^|^GCP_PROJECT=${PROJECT}|PUBLIC_BASE_URL=${PUBLIC_BASE_URL}|SESSION_MINUTES=${SESSION_MINUTES:-30}|PUBLIC_CIDRS=${PUBLIC_CIDRS}|TRUSTED_PROXY_HOPS=${TRUSTED_PROXY_HOPS:-2}|GOOGLE_CLIENT_ID=${GOOGLE_CLIENT_ID}|ADMIN_EMAIL_DOMAIN=${ADMIN_EMAIL_DOMAIN:?set in .env}|PAPI_BASE_URL=${PAPI_BASE_URL:?set in .env}|PAPI_LANG_ID=${PAPI_LANG_ID:-1033}|PAPI_APP_ID=${PAPI_APP_ID:-100}|PAPI_ORG_ID=${PAPI_ORG_ID:-3}|PAPI_API_ACCESS_ID=${PAPI_API_ACCESS_ID:?set in .env}|LOGIN_RATE_MAX=${LOGIN_RATE_MAX:-5}|LOGIN_RATE_WINDOW_MIN=${LOGIN_RATE_WINDOW_MIN:-15}|ENABLE_WHOAMI=${ENABLE_WHOAMI:-0}" \
